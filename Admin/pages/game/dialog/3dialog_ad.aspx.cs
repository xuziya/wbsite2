...
public partial class Admin_pages_game_dialog_3dialog_ad : AdminBasePage
{
    ...
    protected override void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            //触点
            Tools.TreeNode.AddProduceByType(TreeView1.Nodes, "-1", "", 2, false);
            //显示信息
            string id = Request["id"] ?? "0", sole = Request["sole"] ?? "";
            if (oper == "up" && (id != "0" || sole != ""))
            {
                DataTable dt = skin.GetDT("select * from dialog3_info where id=" + id);
                if (dt.Rows.Count > 0)
                {
                    ViewState.Add("id", dt.Rows[0]["id"].ToString());

                    placeSole.Value = dt.Rows[0]["produceSole"].ToString();
                    if (placeSole.Value != "") { placeName.Text = skin.ExecuteScalar("select name from [produce_info] where sole='" + placeSole.Value + "'").ToString(); }
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        if (oper == "add")
        {
            sql.Add(@"insert into dialog3_info values('','" + placeSole.Value + "','" + type1.SelectedValue + "','" + type2.SelectedValue + "','" + content1.Text + "','" + reply1.Text + "','" + point1.Text + "','" + content2.Text + "','" + reply2.Text + "','" + point2.Text + "','" + content3.Text + "','" + reply3.Text + "','" + point3.Text + "','','','','','','',1," + (isUsed.Checked ? 1 : 0) + ",'" + GetSole() + "','" + GetDateTime() + "','" + note.Text + "');");
            sql.Add(harry.Tools.Common.SetSole("dialog3_info"));
        }
        else if (oper == "up")
        {
            sql.Add(@"update [dialog3_info] set [placeSole]='" + placeSole.Value + "',[type1]='" + type1.SelectedValue + "',[type2]='" + type2.SelectedValue + "',[content1]='" + content1.Text + "',[reply1]='" + reply1.Text + "',[point1]='" + point1.Text + "',[content2]='" + content2.Text + "',[reply2]='" + reply2.Text + "',[point2]='" + point2.Text + "',[content3]='" + content3.Text + "',[reply3]='" + reply3.Text + "',[point3]='" + point3.Text + "',isUsed=" + (isUsed.Checked ? 1 : 0) + ",[note]='" + note.Text + "' where [id]=" + ViewState["id"] + ";");
        }
