<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/Admin/master/form.master" CodeFile="4role_menu.aspx.cs" Inherits="Admin_sys_popedom_role_menu" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="body" Runat="Server">
    ...
    <script type="text/javascript">
        ...
        {
            var sole1 = [], sole2 = [];
            tree1.find(null, function (node) { if (node.getChildNodes().length == 0 && node.getChecked()) { sole1.push(node.getValue()) }; });
            if (sole1.length == 0) { sole1.push(tree1.getSelected().getValue()); }
            document.getElementById("<%=tree1_soles.ClientID %>").value = sole1.join(",");
            tree2.find(null, function (node) { if (node.getChecked()) { sole2.push(node.getValue()) }; }); //node.getChildNodes().length == 0 && 
