...
public partial class Admin_sys_popedom_manager_role : AdminBasePage
{
    protected override void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                DataTable dt = skin.GetDT("select convert(bigint,roleSole) as sole from admin_per_manager_role as mr left join admin_per_role as r on mr.roleSole=r.sole where managerSole='" + Request["sole"] + "' order by sole");
            ...
            //树形结构
            Tools.TreeNode.AddManagerByOrgan(TreeView1.Nodes, "-1", "", 5);
    ...
    {
        if (tree1_soles.Value == "") { this.Alert("请选择用户"); return; }
        if (tree2_soles.Value == "") { this.Alert("请选择角色"); return; }
        ...
        {
            sql.Add("delete from admin_per_manager_role where managerSole='" + soles1[i] + "'");
            for (int j = 0; j < soles2.Length; j++)
            {
                sql.Add("insert into [admin_per_manager_role]([sole],[managerSole],[roleSole],[createSole],[createDate],[note]) values('','" + soles1[i] + "','" + soles2[j] + "','" + manager + "','','')");
            }
        }
    ...
    {
        Tools.TreeNode.AddManagerByOrgan(e.Node.ChildNodes, e.Node.Value, " and isUsed=1", 5);
    }
