...
/// <summary>验证基类 - 管理员登录</summary>
public class AdminBasePage : BasePage
{
    /// <summary>本模块根目录</summary>
    public const string PartRoot = "/Admin";
    /// <summary>本模块登陆页面的路径</summary>
    public const string LoginPath = "/Admin/login.aspx";
    /// <summary>本模块中的Cookie、Session前缀</summary>
    public const string SelfPerfix = ProjectPerfix + "admin_";
    ...
    /// <returns></returns>
    protected static ManagerInfo ReloadMessage()
    {
        int state = LoginState(SelfPerfix);
        ManagerInfo model = null;
        switch (state)
        {
            //Session空、Cookie空
            ...
            case 1:
                {
                    model = (ManagerInfo)SessionEx.Get(SelfPerfix + "_model");
            ...
            case 2:
                {
                    ...
                    model = new Skin<ManagerInfo>().Model(uid, pass);//Manager.GetModel(uid, pass);
                    if (!ReloadSession(model)) { Assign("无效的登陆信息:当前登陆管理员账号 [ " + uid + " ]", LoginPath); }
                    break;
                }
            //Session有、Cookie有
            case 3:
    ...
    /// <summary>名称</summary>
    public static string GetName() { ReloadMessage(); return skin.ExecuteScalar("select name from admin_per_manager where sole='" + GetSole() + "'").ToString(); }
    ...
    public static ManagerInfo GetManager() { return ReloadMessage(); }
    ...
    {
        return ReloadSession(new Skin<ManagerInfo>().Model(uid, pass));
    }

    /// <summary>加载Session</summary>
    /// <param name="model">管理员信息</param>
    /// <returns></returns>
    public static bool ReloadSession(ManagerInfo model)
    {
        bool r = model != null;
        if (r)
        {
    ...
    /// <returns></returns>
    public static bool ReloadCookie(ManagerInfo model, DateTime expires)
    {
        bool r = model != null;
        if (r)
        {
            ...
            CookieEx.Add(SelfPerfix + "type", model.OrganSole, expires);
    ...
    {
        return UserCheck(new Skin<ManagerInfo>().Model(uid, pass));
    }

    /// <summary>验证登陆信息</summary>
    /// <param name="model">管理员信息</param>
    /// <returns></returns>
    public static string UserCheck(ManagerInfo model)
    {
        string result = "";
        //用户是否存在
        ...
        //用户使用期
        //else if (Convert.ToDateTime(model.AllowDate).CompareTo(Convert.ToDateTime(DateTime.Now.ToString("yyyy-MM-dd"))) < 0) { result = "管理员账号已过期，请联系系统管理员"; }
        ////检测登陆ip
    ...
    public string GetOperHTML()
    {
        ...
        DataTable dt = skin.GetDT("select distinct menuSole,menuName,menuLinkUrl,menugroups from view_admin_manager_role_menu where menutype='菜单' and mansole='" + mans + "' and menuParent=(select top 1 sole from [admin_per_menu] where string1='" + url + "')");
    ...
    public static string GetPath()
    {
        ...
        if (path.Length > 0)
        {
            ...
            html.Append("<li class='active'><a href='/Admin/main.aspx'>首页</a></li>");
        ...
        return html.ToString().Replace("(管理)","");
    }
}